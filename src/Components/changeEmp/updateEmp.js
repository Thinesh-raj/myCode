...
function ChangeEmp() {
    ...
    useEffect(() => {
        axios.get("http://localhost:3000/employee", { headers: { Authorization: 'Basic U2FtOjIwNTY=' } })
            .then((res) => {
                setEmpData(res.data.find((e) => e.id == obj.id))
            })
    }, [])
    function change(e) {
    ...
    function submit() {
        ...
        if (!check && empData.mobile.length >= 10 && mailCheck.test(empData.email) && !alert3) {
            axios.put("http://localhost:3000/employee/" + obj.id, empData, { headers: { Authorization: 'Basic U2FtOjIwNTY=' } })
    ...
    return <div className="content">
        <div>
            ...
            <p>Gender</p>
            <input type="radio" checked={empData.gender == "M"} onChange={e => setEmpData({ ...empData, gender: "M" })} /><span>M</span>
            <input type="radio" checked={empData.gender == "F"} onChange={e => setEmpData({ ...empData, gender: "F" })} /><span>F</span>
            <p>Course</p>
            <input type="checkbox" checked={empData.course == "MBA"} onChange={e => setEmpData({ ...empData, course: "MBA" })} /><span>MBA</span>
            <input type="checkbox" checked={empData.course == "BSC"} onChange={e => setEmpData({ ...empData, course: "BSC" })} /><span>BSC</span>
            <input type="checkbox" checked={empData.course == "BCA"} onChange={e => setEmpData({ ...empData, course: "BCA" })} /><span>BCA</span>
            <p>Image</p>
